...
class InMemorySpectrogramDataset(Dataset):
    def __init__(self, lmdb_path, labels_map,
        ...
        super(InMemorySpectrogramDataset, self).__init__()
    ...
    def prefetch_buffers(self, lmdb_path):
        ...
        print("[in prefetch_buffers] current_index_range: {} | node_rank: {}".format(current_index_range,
                                                                                     self.local_rank))
        failed_cnt = current_index_range[0]  # repeating indices in case of IndexError
        with env.begin(write=False) as txn:
            for idx in range(current_index_range[0], current_index_range[1]):
                ...
                try:
                    ...
                    byteflow = txn.get(keys[failed_cnt])
                    failed_cnt += 1
                t1 = time.time()
                ...
                if self.precompute_features:
                    ...
                    spec, _ = self.__get_feature__(preprocessed_audio)
                    self.buffered_audios.append(spec)
                else:
                    self.buffered_audios.append(flac_audio)
                self.label_strings.append(lbls)
                if idx % 1000 == 0:
    ...
    def parse_audio_config(self, audio_config):
        ...
        self.precompute_features = audio_config.get("precompute_features", False)
    ...
    def __get_item_helper__(self, index: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        if not self.precompute_features:
            flac_audio = self.buffered_audios[index]
            with io.BytesIO(flac_audio) as buf:
                preprocessed_audio, sr = sf.read(buf)
            real, comp = self.__get_feature__(preprocessed_audio)
        else:
            real = self.buffered_audios[index]
            comp = real
