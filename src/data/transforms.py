...
#             pad_width = ((0, 0), (offset, padding - offset))
#             if self.mode == 'constant':
#                 signal = torch.nn.functional.pad(signal, pad_width[-1], "constant", signal.min())
#                 # signal = np.pad(signal, pad_width,
#                 #                 'constant', constant_values=signal.min())
#             else:
#                 signal = np.pad(signal, pad_width, 'wrap')
...
def get_transforms_v2(train, size,
                      wrap_pad_prob=0.5,
                      spec_num_mask=2,
...
def get_transforms_classifier(train, size,
                              wrap_pad_prob=0.5,
                              spec_num_mask=2,
                              spec_freq_masking=0.15,
                              spec_time_masking=0.20,
                              spec_prob=0.5, center_crop_val=False):
    if train:
        transforms = Compose([
            OneOf([
                PadToSize(size, mode='wrap'),
