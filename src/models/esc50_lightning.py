...
from src.models import causal_resnet
...
class ESC50DataModule(pl.LightningDataModule):
    def __init__(self, hparams):
        super(ESC50DataModule, self).__init__()
    ...
    def setup(self, stage):
        ...
        assert is_lmdb
        from src.data.esc_50_lmdb import InMemoryESC50Dataset

        self.train_set = InMemoryESC50Dataset(self.hparams.cfg['data']['train'],
        ...
        self.val_set = InMemoryESC50Dataset(self.hparams.cfg['data']['val'],
                                            self.hparams.cfg['audio_config'],
                                            fold_index=self.hparams.fold_index,
                                            augment=False,
                                            mixer=None,
...
class ESC50ClassifierPL(pl.LightningModule):
    def __init__(self, hparams):
        super(ESC50ClassifierPL, self).__init__()
        self.hparams = hparams
    ...
    def training_step(self, batch, batch_step):
        ...
        # acc = torch.tensor(correct / y.size(0))
