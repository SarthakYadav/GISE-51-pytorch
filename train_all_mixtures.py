...
from src.data.transforms import get_transforms_fsd_chunks, get_transforms_v2, get_transforms_fsd_mixtures
...
parser.add_argument("--lmdbs_list", type=str,
                    help="list of lmdbs, separated by `;`")

if __name__ == '__main__':
    script_args = parser.parse_args()
    print(script_args)
    lmdbs = sorted(script_args.lmdbs_list.split(";"))
    print(lmdbs)
    for lmdb in lmdbs:
        args = deepcopy(script_args)
        print(lmdb)
        lmdb_name = lmdb.split("/")[-1].replace(".lmdb", "")
        args.output_directory = os.path.join(args.expdir, lmdb_name, "ckpts")
        args.log_directory = os.path.join(args.expdir, lmdb_name, "logs")
