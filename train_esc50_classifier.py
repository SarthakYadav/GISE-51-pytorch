...
from src.models.esc50_lightning import ESC50ClassifierPL, ESC50DataModule
from src.data.transforms import get_transforms_v2, get_transforms_classifier
...
parser.add_argument("--random_clip_size", type=int, default=300)
parser.add_argument("--val_clip_size", type=int, default=500)
...
if __name__ == '__main__':
    ...
    cfg['data'] = data_cfg
    args.cfg = cfg
    ckpt_fd = "{}".format(args.output_directory) + "/{epoch:02d}_{train_loss:.3f}_{val_acc:.3f}"
    ckpt_callback = pl.callbacks.model_checkpoint.ModelCheckpoint(
        filepath=ckpt_fd,
        verbose=True, save_top_k=-1
    )
    es_cb = pl.callbacks.EarlyStopping("val_acc", mode="max", verbose=True, patience=10)
    ...
    val_tfs = get_transforms_classifier(False, args.val_clip_size)

    args.tr_tfs = tr_tfs
    args.val_tfs = val_tfs

    # net = FSD50k_Lightning(args)
    net = ESC50ClassifierPL(args)
    data_module = ESC50DataModule(args)
